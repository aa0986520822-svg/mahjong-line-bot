        ...
        while True:
            try:
                ...
                for (table_id,) in rows:
                    ...
                    # 超時者（仍為 ready）= 視同放棄：直接取消配桌（刪除），避免立刻又被湊桌重倒數
                    timed_out = db.execute("""
                        SELECT user_id FROM match_users
                        WHERE table_id=? AND status='ready' AND expire IS NOT NULL AND expire < ?
                    """, (table_id, now)).fetchall()
                    for (uid,) in timed_out:
                        db.execute("DELETE FROM match_users WHERE user_id=? AND table_id=?", (uid, table_id))
                        try:
                            line_bot_api.push_message(uid, TextSendMessage("⏳ 超過 30 秒未確認，視同放棄，已取消配桌"))
                        except Exception as e:
                            print("timeout push error:", e)

                    # 其餘人（confirmed 或還在桌上）退回等待池
                    remain = db.execute("SELECT user_id FROM match_users WHERE table_id=?", (table_id,)).fetchall()
                    for (uid,) in remain:
                        db.execute("""
                            UPDATE match_users
                            SET status='waiting',
                        ...
                        """, (uid,))
                        try:
                            line_bot_api.push_message(uid, TextSendMessage("⚠️ 有玩家超時未確認，本桌已取消，你已回到等待池"))
                        except Exception as e:
                            print("timeout notify remain error:", e)

                    db.execute("DELETE FROM tables WHERE id=?", (table_id,))
                    db.commit()

                    # ✅ 只嘗試一次重新湊桌（不會包含剛超時被刪掉的人）
                    try:
...
def handle_message(event):
    try:
            ...
            if text == "選單":
                ...
                return True
            # ✅ 暱稱設定（只在這裡提供，不塞進每個按鍵）
            if text in ("設定暱稱", "修改暱稱"):
                user_state[user_id] = {"mode": "set_nick"}
                line_bot_api.reply_message(
                    event.reply_token,
                    TextSendMessage("請輸入暱稱（最多10字）", quick_reply=QuickReply(items=[
                        QuickReplyButton(action=MessageAction(label="🔙 回主畫面", text="選單")),
                    ]))
                )
                return True
            if user_state.get(user_id, {}).get("mode") == "set_nick":
                nick = (text or "").strip()
                if not nick:
                    line_bot_api.reply_message(event.reply_token, TextSendMessage("暱稱不可空白", quick_reply=back_menu()))
                    return True
                nick = nick[:10]
                ...
                line_bot_api.reply_message(
                    event.reply_token,
                    TextSendMessage(f"✅ 已設定暱稱：{nick}", quick_reply=QuickReply(items=[
                        QuickReplyButton(action=MessageAction(label="👤 修改暱稱", text="修改暱稱")),
                        QuickReplyButton(action=MessageAction(label="🔙 回主畫面", text="選單")),
                    ]))
                )
                return True
...
# ✅ admin 最先
        ...
        # === 店家配桌 ===
            ...
            if text.startswith("人數:"):
                ...
                db.commit()
                try_make_table(shop_id, amount)
                # ✅ 若剛好成桌，直接用「回覆」送出按鈕訊息（避免 push 失敗導致看不到按鈕）
                row2 = db.execute("SELECT status, table_id, table_index FROM match_users WHERE user_id=?", (user_id,)).fetchone()
                if row2 and row2[0] == "ready" and row2[1]:
                    table_id2 = row2[1]
                    table_index2 = row2[2]
                    msg = (
                        f"🎉 桌子成立（等待確認）\n"
                    ...
                    line_bot_api.reply_message(event.reply_token, TemplateSendMessage(alt_text="成桌確認", template=template))
                else:
